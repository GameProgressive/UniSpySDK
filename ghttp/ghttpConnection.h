...
#define MAX_RECVFILE_IDLE_TIMEOUT           (30 * 1000) // Set to 30 seconds.
...
typedef struct GHIConnection
{
	...
	GHTTPRequest request;			// This is the object's request index.
	...
	FILE * saveFile;				// If saving to disk, this is the file being saved to.
	char * saveFileName;

	GHTTPBool blocking;				// Blocking flag.
	
	GHTTPBool persistConnection;	// If TRUE, Connection: close will not be sent in the
									// headers and the connection will be left open.
	...
	GHTTPBool userBufferSupplied;	// Set to true if a user buffer was supplied.
	...
	GHTTPBool completed;			// This connection is completed; call the callback and close it.
	...
	GHTTPBool processing;			// If true, being processed. This is used to prevent recursive processing.
	...
	GHIPostingState postingState;	// If posting, this is the state of the upload.
	
	gsi_time maxRecvTime;			// Max time spent receiving per call to "Think"; this prevents blocking on very fast connections.
	char * proxyOverrideServer;		// This allows the use of a different proxy than the global proxy.
...
// This redirects the given connection.
// This resets the connection to get the file at connection->redirectURL.
void ghiRedirectConnection
(
	GHIConnection * connection
);

// This closes all connections and frees up all memory.
